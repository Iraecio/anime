...
export class ImageErrorDebugComponent {
  ...
  readonly isUpdating = computed(() =>
    this.imageErrorService.isCurrentlyUpdating()
  );
  readonly erroredIds = computed(() =>
    this.imageErrorService.getErroredAnimeIds()
  );

  // Só mostra se há erros ou está atualizando
  readonly showDebugInfo = computed(() => {
    if (environment.production === false) {
      return this.errorCount() > 0 || this.isUpdating();
    }
    return false;
  });
